...
//   ① AWENowPlayingInfoCenter setPlayingPlayer:(nil)   放弃"正在播放"身份
//   ② AWENowPlayingInfoCenter setNowPlayingInfo:(nil)  清空抖音侧信息
//   ③ MPNowPlayingInfoCenter  setNowPlayingInfo:(nil)  清空系统侧 → 卡片被系统撤走
//   ④ MPRemoteCommand setEnabled:0 + removeTarget:     摘掉全部远程命令 → 播放按钮消失
// 而【播放态退后台】这一组动作一个都不出现（同一份日志的对照组）→ 所以只有暂停态退后台卡片才掉。
// 会话层排除项：setActive:0 全程零次，根本不是 AVAudioSession 被 deactivate。
//
// 方案：非前台窗口内挡住①~④，让抖音以为一切照旧。第④条是钥匙——只要不让它 removeTarget，
// 抖音自己注册的 play/pause handler 就还挂着，控制中心点播放会直接回调抖音原生逻辑，
...
// 【v4 定位】resignPlayingPlayer: = "辞去正在播放身份"。
// 【v8】入参有三种，实测分布：Foreground 15 / (nil) 15 / AwemeBackgroundPlayModule 7：
//   AWEPlayVideoForegroundRemoteControlManager ← 前台远程控制管理器
//   AWEAwemeBackgroundPlayModule              ← 后台播放模块
//   (nil)                                     ← 通用清理
// 【v9 定案】不再按"入参类名"分流，改为按【App 前后台】分流，与 doExitBackgroundPlayMode 同一判据：
//   前台 = 用户在抖音里滑视频，抖音走【旧的下岗 → 新的上岗 → 发布新信息】的交接链，必须放行；
//   后台 = 用户退回桌面，抖音走【收摊停播】链，此时才拦。
// 反证：v8 只放行 Foreground、其余照拦，实测（diag/v8.log）滑视频后系统侧 SET info 仍彻底停摆
...
// 【v9 关键修正】doExitBackgroundPlayMode：同一条方法、两种语义，必须按前后台区分。
// 之前无差别拦下，把"切视频"这条链一起掐了 —— 铁证（diag/v8.log）：
//   10:25:12 起老板每滑一条视频，抖音必调 doExitBackgroundPlayMode **连中 3 次**，全被我们拦；
//   此后 setCurrentNowPlayingInfo 再没出现过非空值（cnt=0 一路到底）、MPNowPlayingInfoCenter
//   的 SET info 彻底停摆（最后一条 10:25:10）→ 系统侧永远停在那条视频 → 老板实测
...
// ===== 收摊第④步拦截：远程命令层 =====
// 只有 play/pause/toggle 三条命令在托管窗口内被"禁用/解绑"时挡下——
...
// 老板定案：直接去掉左侧亮度/右侧音量两个按钮（AWELandscapeVolumeBrightnessContainer，
// [gscan] 实锤 frame x=59/x=837 两个 36x36 容器）。实测点一下按钮才会激活竖滑调节，
// 按钮没了就点不到、激活不了，滑块永远出不来。锁屏按钮是别的类（gscan 里 x=837,y=141
// 的独立 UIView），不受影响。setHidden: 强制 YES：任何显示尝试都按下去。
%hook AWELandscapeVolumeBrightnessContainer
- (void)setHidden:(BOOL)hidden {
    static NSTimeInterval dyyyLastVolBtnLog = 0;
    NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
...
//   ③ window 兜底：直接加到 window 的浮层全量扫描（addSubview/insertSubview 双入口），
//      严格命中→移除。
//   ④ alert 收尸：UIAlertController viewDidAppear（私有 present 入口的最后必经点），
//      严格命中→dismiss。实测弹窗为抖音自绘 + 私有 present，此层兜住漏网 alert。
//   ⑤ openURL：App Store 域名跳转直接拦。
// 日志只记拦截动作：[upd-classes] 冷启动拦截清单（一次）；[upd-block] 真拦到弹窗/商店页；
//   [upd-badge] 红点。已按老板决定去掉全部"抓漏网"取证留痕（upd-scan/upd-loose/upd-candidates）——
//   弹窗定性为抖音自绘，不再追类名。
// 性能：①启动后异步一次；②~⑤全部事件驱动（present/addSubview/openURL/viewDidAppear 才触发），
...
// 取证扫描核心：对可疑浮层收集文案，命中更新关键词 → 拦截（removeFromSuperview）。
...
static BOOL DYYYForensicScan(UIView *v, NSString *where, BOOL allowRemove) {
    ...
    if (DYYYIsUpdatePopupText(t)) {
        DYYYSpeedDiag([NSString stringWithFormat:@"[upd-block] 拦更新弹窗(%@) class=%@ text=%@",
            where, NSStringFromClass([v class]), t]);
        if (allowRemove) {
...
// 拦截网第四路：UIAlertController 显示成功的最后必经点。
